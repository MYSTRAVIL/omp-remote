...
| `@omp-remote/bridge` | `packages/bridge` | The **OMP extension** installed once into `~/.omp/agent/extensions/`; auto-loads into every session, captures the normalized live feed (`message_*`, `tool_execution_*`, `state`, needs-attention), and injects remote prompt/interrupt. omp also loads it into each `task` subagent (in-process, headless, its session file under the parent's artifacts dir); the bridge never announces those, so they stay part of the parent's run. Fail-safe: a bridge error never crashes the OMP session. Single conduit for adopted **and** spawned sessions. |
